...
public:
	...
	void GenerateMipmap(DX12Context& ctx, const std::span<DX12Image> images) const; // Batch
	void GenerateMipmap(DX12Context& ctx, DX12Image* image) const; // Single image

	void Update(DX12Context& ctx, UIData& uiData) override {}
	void PopulateCommandList(DX12Context& ctx) override {};

private:
	uint32_t GetHeapSize(const std::span<DX12Image> images) const;
